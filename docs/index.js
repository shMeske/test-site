...
async function startApplication() {
  ...
  const env_spec = ['https://cdn.holoviz.org/panel/wheels/bokeh-3.3.0-py3-none-any.whl', 'https://cdn.holoviz.org/panel/1.3.1/dist/wheels/panel-1.3.1-py3-none-any.whl', 'pyodide-http==0.2.1', 'https://raw.githubusercontent.com/shMeske/WheelFiles/master/docopt-0.6.2-py2.py3-none-any.whl', 'https://raw.githubusercontent.com/shMeske/WheelFiles/master/littleutils-0.2.2-py3-none-any.whl', 'fau_colors==1.5.3','https://raw.githubusercontent.com/shMeske/WheelFiles/master/pingouin-0.5.4-py3-none-any.whl', 'https://raw.githubusercontent.com/shMeske/WheelFiles/master/zope.interface-6.2.dev0-py3-none-any.whl', 'https://raw.githubusercontent.com/shMeske/WheelFiles/master/biopsykit-0.9.0-py3-none-any.whl', 'datetime', 'seaborn', 'matplotlib','nilspodlib', 'numpy', 'packaging', 'pandas', 'param', 'pathlib', 'plotly', 'pytz',  'typing', 'typing_extensions','holoviews', 'mne']
...
class TimesToSubject(pn.viewable.Viewer):
    ...
    def add_new_subject(self, target, _):
        if (
            ...
            app.notifications.error("Subject already added")
            return
...
class LoadSalivaData(SalivaBase):
    ...
    def parse_file_input(self):
        ...
        try:
            ...
            app.notifications.success("Files uploaded")
        except Exception as e:
...
class SetSleepDataParameters(SleepBase):
    ...
    def parameter_changed(self, target, event):
        if self.selected_device == "":
            app.notifications.error("No device selected")
            return
        ...
        app.notifications.success(f"{target} changed to {event.new}")
...
class DownloadResults(PhysiologicalBase):
    ...
    def get_selected_files(self):
        ...
        app.notifications.info("Loading Results")
        ...
        app.notifications.info("Results loaded")
...
class AddTimes(PhysiologicalBase):
    ...
    def parse_time_file(self, target, event):
        ...
        if self.df is None:
            app.notifications.error("Could not parse the given time File")
            return
        if not self.ready:
    ...
    def parse_file(self, file_name, file_content) -> pd.DataFrame:
        if file_content is None:
            app.notifications.error("No file content")
            return pd.DataFrame()
        if ".csv" in file_name:
    ...
    def check_subject_condition_columns(self, df):
        if "subject" not in df.columns:
            app.notifications.error("Subject column must be specified")
            return False
        if "condition" not in df.columns:
            app.notifications.error("Condition column must be specified")
            return False
        return True
    ...
    def parse_file_input(self, _, event):
        ...
        try:
            match fileType:
                ...
                case _:
                    app.notifications.error("No matching parser found")
                    self.ready = False
                    return
            # if self.ready:
            #     app.notifications.success(
            ...
            #     )
            # else:
            #     app.notifications.error("File upload failed", duration=5000)
        except Exception as e:
            self.ready = False
            app.notifications.error(f"File upload failed: {str(e)}", duration=5000)
...
class DataArrived(PhysiologicalBase):
    ...
    def panel(self):
        ...
        if self.sampling_rate > 0:
            ...
            self.ready = False
            app.notifications.warning("Please provide a sampling rate")
...
class ProcessingAndPreview(PhysiologicalBase):
    ...
    def process_rsp(self):
        if self.sampling_rate == -1:
            app.notifications.error("Bitte eine sampling Rate eingeben")
            return
...
class ConvertScales(QuestionnaireBase):
    ...
    def apply_questionnaire_scale(self, target, _):
        ...
        if key is None or not isinstance(key, str):
            app.notifications.warning("No Questionnaire selected")
            return
        if offset is None or not isinstance(offset, int):
            app.notifications.warning("No offset selected")
            return
        ...
        try:
            ...
            app.notifications.error(f"Validation Error: {e}")
    ...
    def apply_column_scale(self, target, _):
        ...
        if cols is None or len(cols) == 0:
            app.notifications.warning("No Columns selected")
            return
        offset = target[1].value
        if offset is None or not isinstance(offset, int):
            app.notifications.warning("No offset selected")
        ...
        if any([col not in self.data.columns.to_list() for col in cols]):
            app.notifications.warning("Not all columns are in the data")
            return
        try:
            self.data_scaled = bp.questionnaires.utils.convert_scale(
                self.data, cols=cols, offset=offset
            ...
            app.notifications.error(f"Key Error: {ke}")
...
class CropScales(QuestionnaireBase):
    ...
    def crop_data(self, target, event):
        ...
        try:
            ...
            app.notifications.error(f"Error while cropping the data: {e}")
...
class InvertScores(QuestionnaireBase):
    ...
    def invert_scores(self, target, event):
        if self.data is None:
            app.notifications.error("No data loaded")
            return
...
class SuggestQuestionnaireScores(QuestionnaireBase):
    ...
    def add_questionnaire(self, selected_questionnaire, _):
        ...
        if questionnaire is None or questionnaire == "" or self.data is None:
            app.notifications.error("No Questionnaire selected")
            return
        i = 0
    ...
    def rename_questionnaire(self, target, event):
        ...
        if score not in list(
            ...
            app.notifications.error(f"Questionnaire: {score} not supported")
            return
...
class ConvertToLong(QuestionnaireBase):
    ...
    def convert_to_long(self, target, event):
        ...
        try:
            ...
            app.notifications.error(f"The error {e} occurred")
